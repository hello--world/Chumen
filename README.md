...
- `Contents/MacOS/Chumen`
- `Contents/Resources/ChumenHelper`
...
- 累计流量：来自 mihomo `/connections` 或 `/traffic` 暴露的总上传/下载累计。
- 当前速率：来自 mihomo `/traffic` 流的实时上传/下载速率。
- 累计分流：Chumen 根据 `/connections` 中每条连接的 `chains` 字段，在本次运行观察窗口内按代理和直连累加连接增量。mihomo 不直接提供历史代理/直连总账，所以这项不是对既有总累计的回溯拆分。

运行配置生成流程：

1. 读取当前启用的 profile YAML。
2. 删除由 Chumen 接管的顶层键，避免用户配置与 GUI 设置互相覆盖。
3. 追加 Chumen 生成的端口、controller、TUN、DNS 等运行项。
4. 写入 `chumen-runtime.yaml`。
5. 启动前先执行 `mihomo -t` 验证配置。
6. 启动或通过 API reload 当前配置。
