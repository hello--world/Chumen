...
scripts/build_app.sh          构建 debug/release App
...
debug 包默认使用另一组端口，避免和正式包同时运行时冲突：
...
debug 包的 DNS listen 默认是 `127.0.0.1:1153`，TUN 设备默认是 `utun1025`；正式包分别保持 `127.0.0.1:1053` 和 `utun1024`。
