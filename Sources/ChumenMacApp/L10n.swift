...
enum L10n {
    ...
    private static let zhHans: [Key: String] = [
        ...
        .pinDescription: "默认 PIN 参与保护配置密钥，但不锁屏；开启锁屏后启动才需要输入 PIN。",
        .pinInfoConfigEncrypted: "配置始终加密，避免订阅和节点明文被扫描。",
        .pinInfoPINProtectsKey: "PIN 参与保护 age 私钥；默认自动解密，不锁应用。",
        ...
        .pinSetupRequired: "默认 PIN 参与保护配置密钥；应用锁屏是独立选项，默认关闭。",
    ...
    private static let en: [Key: String] = [
        ...
        .pinDescription: "PIN protects the config key by default, but app lock stays off unless enabled.",
        .pinInfoConfigEncrypted: "Configs stay encrypted to avoid plaintext proxy scans.",
        .pinInfoPINProtectsKey: "PIN protects the age private key; the app auto-unlocks by default.",
        ...
        .pinSetupRequired: "PIN protects the config key by default; app lock is separate and defaults off.",
